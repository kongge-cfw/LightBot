<template>
  <div
    v-if="!msg._streaming && hasActionContext"
    class="message-actions"
    :class="{ 'is-error-actions': isErrorProfile }"
  >
    <a-tooltip
      v-if="showTts"
    ...
    <a-tooltip v-if="showReply" title="引用回复">
    ...
    <a-tooltip v-if="showStar" :title="msg._starred ? '取消收藏' : '收藏'">
...
import { getMessageActionProfile } from '../../../utils/chat/messageErrorState.js'
...
const actionProfile = computed(() => getMessageActionProfile(props.msg))
const isErrorProfile = computed(() => actionProfile.value.startsWith('error_'))

const showTts = computed(() => props.msg.role === 'assistant'
  && props.showTtsBtn
  && actionProfile.value === 'normal')
const showRegenerate = computed(() => props.msg.role === 'assistant'
  && props.canRegenerate
  && (actionProfile.value === 'normal' || isErrorProfile.value))
const showRequestId = computed(() => props.msg.role === 'assistant'
  && !!props.msg._requestId
  && actionProfile.value === 'error_persisted')
const showMessageId = computed(() => props.msg.role === 'assistant'
  && !!props.msg._id
  && actionProfile.value === 'error_persisted')
const showViewRaw = computed(() => props.msg.role === 'assistant'
  && (actionProfile.value === 'normal' || isErrorProfile.value))
const showEdit = computed(() => props.msg.role === 'user'
  && !props.loading
  && props.isLastUserMessage
  && !props.msg._replyToMessageId)
const showReply = computed(() => props.msg.role === 'assistant' && actionProfile.value === 'normal')
const showFeedback = computed(() => props.msg.role === 'assistant'
  && actionProfile.value === 'normal'
...
.message-actions.is-error-actions {
  margin-top: 4px;
}
.btn-action-text {
  display: inline-flex;
