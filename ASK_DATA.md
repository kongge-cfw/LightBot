...
> 数据池语义分析 · 对话式查数 / 统计 / 下钻  
> 状态：**主路径 = Agent 勾选数据模型即可问** · 2026-07-31  
...
```text
主路径：数据模型写好字段中文名/说明 → Agent「可问数据」勾选 → 即可问数
...
| **建设者** | 维护数据模型与数据池（字段中文名/说明）；在 Agent「可问数据」勾选模型；平台内调试问数 |
...
| Agent 详情 → **可问数据** | **勾选数据模型**即开启问数（自动 ensure 语义 + 画像）；**不绑定** PgSqlTool |
...
2. Agent「可问数据」勾选该模型 → 后台自动维度/默认指标/画像 → 即可问数
...
| Agent 绑定 | 「可问数据」，与「知识库」并列，避免写成「数据库权限」 |
...
1. 建设者能为至少一个业务模型写好字段语义，并在 Agent「可问数据」勾选。  
...
- [x] Agent 可绑定「可问数据 / 数据模型」，与知识库绑定可区分
