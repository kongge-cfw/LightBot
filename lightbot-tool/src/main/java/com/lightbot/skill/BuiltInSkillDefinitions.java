    ...
    private static final List<Definition> DEFS = List.of(
            new Definition(
                    ...
                    - `delegate_to_subagent`：并行派发子任务的委派工具。仅在当前 Agent 已绑定子智能体时可用；一次调用可传多个 `tasks`，`mode="parallel"` 并行。**优先派发给这两类子智能体**（若已绑定）：
                      - `research-agent`（深度研究员）：围绕单个明确子问题做多轮联网/知识库检索，返回按要点组织、附来源 URL 的结构化发现。**这是主力，按子问题并行多开。**
                    ...
                    - `sandbox_write_file` / `sandbox_append_file` + `present_artifacts`（自动注入）：仅在用户要求可下载文件时，主 Agent 将最终报告写入 `outputs/reports/*.md` 再登记。中间文件写普通工作区路径。
                    ...
                    ### 2. 规划拆解
                    用 `write_todos` 把研究目标拆成**可独立调研**的子问题（2~8 项完整快照），每个子问题写明产出标准（要回答什么、需要哪类证据）。子问题应正交、覆盖完整，避免重叠或遗漏关键角度。同一时刻仅一个主待办为 `in_progress`。

                    ### 3. 并行派发调研
                    - 把互不依赖的子问题在**一次 `delegate_to_subagent`** 调用中通过 `tasks` 并行派发给 `research-agent`，`mode="parallel"`。
                    - 每个 task 写清：子问题目标、已知上下文、期望输出格式（要点 + 来源 URL 列表 + 证据置信度）。
                    ...
                    ### 4. 核验关键结论
                    对**影响最终结论的关键论断**、数字，以及子智能体之间相互冲突的发现，派发 `fact-verifier` 做对抗式核验。默认「证据不足即标注存疑」。核验未通过的结论不要写进正文，或必须明确降级标注。
                    ...
                    ## 输出约束
                    - 最终交付一份可直接使用的报告，而非「我打算怎么研究」。
                    - 不外泄中间推理过程、原始检索日志，也不要把待办清单原样输出成正文。
                    - 报告语言与用户提问语言一致，使用正式、克制、可复核的书面表达。
                    - 仅当用户明确要求可下载文件时，用 `sandbox_write_file` 写入 `outputs/reports/research-report.md`（长报告可用 `sandbox_append_file` 分章节续写），再用 `present_artifacts` 登记。
                    """,
                    10
            ),
            new Definition(
