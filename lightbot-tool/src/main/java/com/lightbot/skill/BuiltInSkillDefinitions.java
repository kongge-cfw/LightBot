...
public final class BuiltInSkillDefinitions {
    ...
    private static final List<Definition> DEFS = List.of(
            new Definition(
                    ...
                    List.of("web_search", "ask_user", "ocr_parse_file", "sandbox_read_file", "sandbox_write_file"),
                    List.of(),
                    """
                    ...
                    **本技能只使用 LightBot 当前实际可用的工具**：
                    ...
                    - 仅在用户明确要求可下载文件时，用 `sandbox_write_file` 写入 `outputs/` 下的 Markdown/文本交付物，再用 `present_artifacts` 登记。中间文件写普通工作区路径，不得交付 Skill 文件。
                    ...
                    **四、交付**：
                    ...
                    - 存在失败、未完成任务或证据冲突时必须如实说明影响；只有用户要求文件交付时，主 Agent 才先用 `sandbox_write_file` 写入例如 `outputs/reports/research-report.md`，确认成功后再使用 `present_artifacts` 登记并告知用户。
