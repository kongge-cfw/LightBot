...
public class ModelProviderServiceImpl extends ServiceImpl<ModelProviderMapper, ModelProvider>
    ...
    @Override
    public List<ProviderWithModelsVO> listWithModels(String modelType) {
        // 1. 取所有启用提供商
        List<ModelProvider> providers = listAllActive();
        // 2. 按提供商拉取模型，按 modelType code 过滤；过滤后无模型的提供商不返回
        List<ProviderWithModelsVO> result = new java.util.ArrayList<>();
        for (ModelProvider p : providers) {
            List<Model> models = modelService.listByProviderId(p.getId());
            if (modelType != null && !modelType.isBlank()) {
                models = models.stream()
                        .filter(m -> m.getType() != null && modelType.equalsIgnoreCase(m.getType().getCode()))
                        .toList();
            }
            if (!models.isEmpty()) {
